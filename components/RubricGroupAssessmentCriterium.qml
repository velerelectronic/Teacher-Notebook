...
Item {
    ...
    signal editRubricAssessmentDescriptor(int individual)

    signal rubricGroupAssessmentDescriptorSelected()

    onCriteriumChanged: lastScoresModel.refresh()
    onAssessmentChanged: lastScoresModel.refresh()

    Models.RubricsLastScoresModel {
        id: lastScoresModel

        filters: ['assessment=?', 'criterium=?']

        function refresh() {
            if ((assessment>-1) && (criterium>-1)) {
                bindValues = [assessment, criterium];
...
//                            onHeightChanged: criteriaList.descriptorsCriteriumHeight = height
    ...
    Component.onCompleted: {
        console.log('crt', criteriumAssessmentEditor.criterium);
    }
}
